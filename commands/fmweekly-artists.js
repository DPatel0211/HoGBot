...
module.exports.run = async (client, message, args) => {
    require('dotenv').config();
      ...
      try {
        ...
        let count = result.topartists.artist.length;
        ...
        for (let i = 0; i < count; i++) {
          ...
          if (artist.image[artist.image.length - 1]["#text"] !== '') {
            const artistArt = await loadImage(artist.image[artist.image.length - 1]["#text"]);
        ...
        message.channel.send(`${message.author}'s 3x3 Top Artists Chart of the Week (via Last.fm: https://www.last.fm/user/${associationUser.lastFMUsername})`, attachment).then(async embedMessage => {
